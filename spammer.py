...
def Main():
	...
	if int(version) > ver:
		info = Back.RED+"\nเวอร์ชันล้าสมัยและจำเป็นต้องได้รับการอัปเดต"+Style.RESET_ALL
	else:
		info = " "
	logo = Fore.GREEN+'''
S͓̽c͓̽r͓̽i͓̽p͓̽t͓̽k͓̽i͓̽n͓̽g͓̽ ͓̽S͓̽C͓̽K͓̽'''+Style.RESET_ALL
	def main():
		global info
		while True:
			os.system("clear")
			...
			print ('''
1) ยิงSMS +ยิงรัวๆ
2) เพิ่มหมายเลขกันโดนแสปม antispam
3) ตรวจสอบหมายเลขกันโดนสแปม antispam
4) อัพเดตไหม่
5) Exit.

พิมเลือกตามความอยากรู้ได้เลย  :''')
			input1 = input(Fore.BLUE+" 👉 > "+Style.RESET_ALL)
			if input1 == "1":
				print("ใส่เบอร์ (ตัวอย่าง +66ตัด0ผออก):")
				phone = input(Fore.BLUE+"  👉 > "+Style.RESET_ALL)
				try:
					if int(phone):
						print(" ใส่จำนวนส่ง (เท่าไรก็แล้วแต่):")
						count = input(Fore.BLUE+" 👉 > "+Style.RESET_ALL)
						try:
							if int(count):
								count=int(count)
								_phone=phone
								...
								if int(id) > 0:
									info = Fore.RED+"\nหมายเลขโทรศัพท์อยู่ในรายการป้องกันสแปม."+Style.RESET_ALL
									main()
								elif int(id)==0:
									_name = ''
									for x in range(12):
									...
									email = _email
									info = '\nเบอร์: {}\nรอบ: {}'.format(phone, count)+'\nผู้ส่งสแปมเปิดตัว.\nหากต้องการหยุด กด Ctrl+Z.'
									os.system("clear")
									print(logo)
									print(info)
									while iteration < count:
										...
										try:
											...
											pass
										iteration += 1
										print(('{} วนลูป.').format(iteration))
									info = Fore.BLUE+"\nเสร็จสิ้น.\nเบอร์โทรศัพท์: {}\nจำนวนรอบที่เสร็จสมบูรณ์: {}".format(phone, iteration)+Style.RESET_ALL
									main()

						except:
							info = Fore.RED+"\nจำนวนที่ป่อนผิด".format(phone)+Style.RESET_ALL
							main()
				except:
					info = Fore.RED+"\nป้อนหมายเลขโทรศัพท์ไม่ถูกต้อง".format(phone)+Style.RESET_ALL
					main()
			...
			if input1 == "2":
				...
				try:
					if int(phone):
						...
						if int(id) > 0:
							info = Fore.GREEN+"\nหมายเลข {} มีอยู่ในกันแสปมแล้ว.".format(phone)+Style.RESET_ALL
							main()
						elif int(id) == 0:
							requests.post('https://rainbow-t-shirt.com/spymer/ajax.php', data={'phone': phone})
							info = Fore.GREEN+"\nหมายเลข {} เพิ่มไปยังกันแสปม.".format(phone)+Style.RESET_ALL
							main()
				except:
					info = Fore.RED+"\nป้อนหมายเลขโทรศัพท์ไม่ถูกต้อง!".format(phone)+Style.RESET_ALL
					main()
				...
				try:
					if int(phone):
						...
						if int(id) > 0:
							...
							info = Fore.RED+"\nหมายเลข {} ไม่มีการกันแสปม.".format(phone)+Style.RESET_ALL
							main()
				except:
					info = Fore.RED+"\nป้อนหมายเลขโทรศัพท์ไม่ถูกต้อง!".format(phone)+Style.RESET_ALL
					main()
			...
			if input1 == "4":
				cmd = os.system("cd && rm -rf ~/spymer && git clone https://github.com/scriptkingSCK/spamSMS && sh ~/spamSMS/install.sh")
				exit()
